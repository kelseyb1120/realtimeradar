...
def save_latest_kamx_z_png_s3(radar):
    ...
    plt.savefig(fn)
    s3_key = 'kamx_irma/' + gen_s3_key(radar_start_date,
                                                 'kamx_z_')
    s3 = boto3.resource('s3')
    data = open(fn, 'rb')
    s3.Bucket('openradar').put_object(Key=s3_key, Body=data, ACL='public-read')
    data.close()
    data2 = open(fn, 'rb')
    s3.Bucket('openradar').put_object(Key='latest_kamx_z.png', Body=data2, ACL='public-read')
    data2.close()
    return s3_key, fn

def save_latest_kamx_kdp_png_s3(radar):
    radar_start_date = netCDF4.num2date(radar.time['data'][0],
        radar.time['units'])
    local_fig =  tempfile.NamedTemporaryFile(suffix='.png')
    fn = local_fig.name
    plot_radar_ppi(radar, field = 'specific_differential_phase', cmap=pyart.graph.cm.LangRainbow12,
               vmin=-0.1, vmax=3, sweep=0, fig=None)
    plt.savefig(fn)
    s3_key = 'kamx_irma/' + gen_s3_key(radar_start_date,
                                                 'kamx_kdp_')
    s3 = boto3.resource('s3')
    ...
    data.close()
    data2 = open(fn, 'rb')
    s3.Bucket('openradar').put_object(Key='latest_kamx_kdp.png', Body=data2, ACL='public-read')
    data2.close()
    return s3_key, fn

def save_latest_kamx_zdr_png_s3(radar):
    radar_start_date = netCDF4.num2date(radar.time['data'][0],
        radar.time['units'])
    local_fig =  tempfile.NamedTemporaryFile(suffix='.png')
    fn = local_fig.name
    plot_radar_ppi(radar, field = 'differential_reflectivity', cmap=pyart.graph.cm.LangRainbow12,
               vmin=-0.1, vmax=3, sweep=0, fig=None)
    plt.savefig(fn)
    s3_key = 'kamx_irma/' + gen_s3_key(radar_start_date,
                                                 'kamx_zdr_')
    ...
    data.close()
    data2 = open(fn, 'rb')
    s3.Bucket('openradar').put_object(Key='latest_kamx_zdr.png', Body=data2, ACL='public-read')
    data2.close()
    return s3_key, fn

def save_latest_kamx_vr_png_s3(radar):
    radar_start_date = netCDF4.num2date(radar.time['data'][0],
        radar.time['units'])
    local_fig =  tempfile.NamedTemporaryFile(suffix='.png')
    fn = local_fig.name
    plot_radar_ppi(radar, field = 'velocity', cmap=pyart.graph.cm.NWSVel,
               vmin=-40, vmax=40, sweep=1, fig=None)
    plt.savefig(fn)
    s3_key = 'kamx_irma/' + gen_s3_key(radar_start_date,
                                                 'kamx_vr_')
    s3 = boto3.resource('s3')
    ...
    data.close()
    data2 = open(fn, 'rb')
    s3.Bucket('openradar').put_object(Key='latest_kamx_vr.png', Body=data2, ACL='public-read')
    data2.close()
    return s3_key, fn
...
def gen_s3_key(fig_datetime, pref, sfx=''):
    ...
    o_str = datetime.datetime.strftime(fig_datetime, fmt_str)
    return o_str
